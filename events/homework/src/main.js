...
// При натисканні Ctrl +, замість textarea з'являється div з уже зміненим текстом. Не забудь вимкнути поведінку за замовчуванням для цих поєднань клавіш.
...
const mainDiv = document.getElementById("content"); //ствоюємо id 'content'

const tableDiv = document.createElement("div"); //ствоюємо div
tableDiv.setAttribute("id", "content__tableDiv"); // назначаємо атрибут
tableDiv.className = "content__tableDiv"; //ствоюємо клас для div

const tableTag = document.createElement("table"); //створюємо елемнет table
tableTag.setAttribute("id", "content__table");  // для table пишемо id
tableTag.className = "content__table"; //назначаємо йому клас

tableDiv.append(tableTag); // визивємо 2 функіції нових елементів щоб вони з'явились
mainDiv.append(tableDiv);

//тут створюємо масив - клас
...
const arrEmp = [
  ...
  new Employee("Chris Rea", "Manager", "Sales", 2000),
...
//ще один клас для таблиці
class EmpTable {
  constructor(arr) {
    this.arr = arr;
  }

  //ще один клас для таблиці створюємо нові елементи в яких буде міститись дані з масиву
  getHtml() {
...
//визиваємо ці функції
    ...
    head.append(th4);
    tbody.append(head);
    //назначаємо стилі та додаємо в кінці стобчики
    for (let i in array) {
    ...
    table.setAttribute("style", "margin: auto; background-color: beige; border: 2px solid brown");
  }
}
